...
class CustomUserManager(BaseUserManager):
    def create_user(self, phonenumber, fullName=None, email=None, password=None, **extra_fields):
        if not phonenumber:
            raise ValueError('Users must have a phonenumber')
        user = self.model(phonenumber=phonenumber, email=email, fullName=fullName)
